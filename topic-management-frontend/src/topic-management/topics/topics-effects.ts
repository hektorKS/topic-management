...
import {
  ...
  loadTopic,
  loadTopicsInBucket,
  newTopicButtonSelected, newTopicSaved, saveNewTopic,
  topicDeleted,
  topicLoaded,
  topicSelected,
...
import {selectBucketId, selectSchoolId} from "../topic-management-router-state";
...
export class TopicsEffects {
  ...
  newTopic$: Observable<Action> = createEffect(() => {
    return this.actions$.pipe(
      ...
      flatMap(([_, bucketId]) => {
        return this.router.navigate(['buckets', bucketId, 'new-topic'])
          .then(_ => changeBreadcrumb({
              name: "New topic",
              url: this.router.url
            })
